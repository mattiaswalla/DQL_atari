...
def atari_model(n_actions):
    ...
    prev_conv_input = keras.layers.Input((3,64), name='mask')
    ...
    concatenated_conv = keras.layers.concatenate([conv_flattened, prev_conv_input])
    
    # "The final hidden layer is fully-connected and consists of 256 rectifier units."
    hidden = keras.layers.Dense(256, activation='relu')(concatenated_conv)
    # "The output layer is a fully-connected linear layer with a single output for each valid action."
    output = keras.layers.Dense(n_actions)(hidden)
    # Finally, we multiply the output by the mask!
    filtered_output = keras.layers.multiply([output, actions_input])

    model = keras.models.Model(input=[frames_input, actions_input], output=[filtered_output, conv_flattened])
...
def get_epsilon_for_iteration(iteration):
    ...
    if iteration < LIMIT_1 :
        ...
        return VALUE_1 +(iteration -LIMIT_1)/(LIMIT_2 - LIMIT_1)*(VALUE_2 - VALUE_1)
    else:
        return VALUE_2
...
def q_iteration(env, model, state, iteration, memory):
    ...
    if random.random() < epsilon:
        action, output = choose_best_action(model, state)
    ...
    BATCH_SIZE = 32
    LIMIT_1 = 10000
    if (iteration > BATCH_SIZE and iteration > LIMIT_1):
...
def main():
    model = atari_model(4)
    #model = keras.models.load_model("model_weights_2018_05_09_feedback.HDF5")
